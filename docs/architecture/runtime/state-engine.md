...
Turns ordered state changes into canonical state, roots, snapshots, proofs, and restoration material.

## Responsibilities

- Own the canonical description of state engine within the architecture book.
...
- store owns key/value state.
- apply mutates state and emits snapshots.
- merkle computes roots and hex encodings.
- proof/history/snapshot support verification and persistence.
...
Production foundation for deterministic state; persistence integration varies by host path.
