...
Owns deterministic protocol primitives, state hashing, receipts, replay, checkpoints, lineage, world runtimes, and exported ABI entry points.

## Responsibilities

- Own the canonical description of execution core within the architecture book.
...
- execution-core/src/lib.rs exposes protocol modules and WASM ABI shims.
- execute.rs computes previous/new roots, execution roots, state output hashes, and receipt hashes.
- state_engine contains apply, history, Merkle, proof, snapshot, and store modules.
- hashing/canonical/codec modules define canonical bytes and digest boundaries.
...
Production foundation with many scaffold-adjacent higher-level domains.
