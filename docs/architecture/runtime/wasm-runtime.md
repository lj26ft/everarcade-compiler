...
Defines deterministic guest execution boundaries for contract packages and game logic.

## Responsibilities

- Own the canonical description of wasm runtime within the architecture book.
...
- alloc/execute/output_len exports guest ABI.
- everarcade_abi serializes VmInput and VmOutput.
- wasm reports validate isolation, fuel metering, checkpoint restore, and replay.
- Host package loader and runner invoke package execution.
...
Foundational ABI exists; production package registry and hardened host policies remain roadmap items.
