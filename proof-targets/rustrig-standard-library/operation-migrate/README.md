# Proof Target: operation.migrate

Maturity: CANDIDATE

This target defines the verifier handoff for `operation.migrate`. It describes state, input, validation, transition, receipt, replay, and root behavior without claiming formal proof for the full standard library.

## Receipt accumulator requirement

Migration verifiers must derive `receipt_root` from bundle data only. The trusted input is the receipt accumulator:
