...
Define the gap between the current deterministic foundation and a production-operated multi-node platform.

## Responsibilities

- Own the canonical description of production gaps within the architecture book.
...
- Persistence: durable state, receipt, archive, checkpoint, and projection stores need selected backends and retention policy.
- Registry: package discovery, signing, compatibility, and rollback are not yet complete.
- Federation: WAN transport, backpressure, malicious peer handling, and topology control need hardening.
- Operations: release gates should become enforced CI/CD and operator commands.
- Renderer: projection/history/federation are scaffold-level and must remain non-authoritative.
...
Close gaps by converting reports into executable gates, stabilizing APIs, and demoting any unverified milestone claim.
