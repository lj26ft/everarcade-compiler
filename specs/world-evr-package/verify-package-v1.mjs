...
function verify(dir) {
  ...
  assert(manifest.spec_version === 'V1', 'manifest.spec_version must be V1');
  ...
  assert(manifest.runtime.runtime_version === runtime.runtime_version, 'manifest.runtime.runtime_version must equal runtime.runtime_version');
  ...
  if (existsSync(join(dir, 'restore/checkpoint.json'))) {
    ...
    assert(checkpoint.roots?.continuity_root === sha(dir, 'restore/journal.json'), 'checkpoint.roots.continuity_root does not equal recomputed continuity root');
  }
