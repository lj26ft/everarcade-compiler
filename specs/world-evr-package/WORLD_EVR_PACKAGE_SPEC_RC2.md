# World Package Specification RC2

## Status

`WORLD_EVR_PACKAGE_SPEC_RC2` is the second review candidate for the `world.evr` package format. It fixes the RC1 binding gaps found by independent verification. RC2 is **not** frozen package format v1.

## Authoritative Package Hash

For RC2, `hash-manifest.json` is authoritative. Older Rust envelope or `execution-core/src/codec/package_encode.rs` descriptions are non-authoritative for `world.evr` unless they are rewritten to produce byte-identical `hash-manifest.json` output and the same package hash stream.
...
`hash-manifest.json` and `expected-package-hash.txt` are verifier control files and are not entries in `hash-manifest.files` for the RC2 directory fixtures.

### Manifest Binding

RC2 fixtures use the JSON schema in `fixtures/world-package-valid-001/manifest.json`. The canonical package identifier is `manifest.package_name`.
...
manifest.runtime.version == runtime/runtime.json.version.
...
A verifier MUST reject a package when `manifest.runtime.runtime_id` differs from `runtime/runtime.json.runtime_id` or when `manifest.runtime.version` differs from `runtime/runtime.json.version`, even after rebuilding `hash-manifest.json`.
...
RC2 fixtures do not include migration artifacts; these predicates are mandatory for any package that does.
...
Inline certification files included in `hash-manifest.files` cannot store their own final package hash without self-reference. For inline RC2 fixture certification, `expected-package-hash.txt` is the package-hash binding control.

## Failure Fixtures

The RC2 verifier MUST accept `fixtures/world-package-valid-001/` and reject every directory under `failure-fixtures/`. In particular:
