...
REPORT="deployment/reports/two_node_certification_report.md"
echo "[everarcade] two-node certification: ${CARGO_FLAGS[*]:-default cargo resolution}"
CARGO_BUILD_JOBS="$CARGO_BUILD_JOBS" cargo test -p execution-core --test two_node_certification_tests "${CARGO_FLAGS[@]}"
cat > "$REPORT" <<'REPORT'
