...
REPORT_PATH="$REPORT_DIR/clean_clone_certification_report.txt"
...
Timestamp: $timestamp
Vendor Artifact Present: $vendor_artifact_present
...
print_summary() {
  printf 'Clean Clone Certification: %s\n' "$overall_result"
  printf 'Report: %s\n' "$REPORT_PATH"
}
