...
impl ReplayEquivalenceRuntime {
    pub fn compare_streams(source: &ReplayTransportStream, observer: &ReplayTransportStream) -> ReplayEquivalenceResult {
        ...
        if source.accepted.len() != observer.accepted.len() {
            return ReplayEquivalenceResult { equivalent: false, reason: "chunk_count_mismatch".to_string(), proof };
        }
        if proof.source_tail_hash != proof.observer_tail_hash {
            return ReplayEquivalenceResult { equivalent: false, reason: "continuity_hash_mismatch".to_string(), proof };
        }
        ReplayEquivalenceResult { equivalent: true, reason: "equivalent".to_string(), proof }
    }
}
