...
impl ProjectionReplayRuntime {
    pub fn new(frames: Vec<RenderProjectionArtifact>) -> Self { Self { frames } }
    ...
    pub fn step(&self, cursor: &mut ProjectionReplayCursor) -> Option<RenderProjectionArtifact> {
        ...
        if frame.is_some() { cursor.index += 1; }
        frame
    }

    pub fn window(&self, window: &ProjectionReplayWindow) -> Vec<RenderProjectionArtifact> {
        self.frames.iter().filter(|f| f.frame_index >= window.start_frame && f.frame_index <= window.end_frame).cloned().collect()
    }
}
