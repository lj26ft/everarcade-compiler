...
pub fn identity_join(input: Input, state: State) -> Result<Output, Error> {
    apply_named("identity.join", input, state)
}
pub fn identity_leave(input: Input, state: State) -> Result<Output, Error> {
    apply_named("identity.leave", input, state)
}
