...
export function applyArenaInput(state, envelope) {
  ...
  if (action.action === 'attack') {
    ...
    player.score += 10;
    const event = { tick: after.tick + 1, attacker: action.player, target: action.target, damage: 25, target_health: target.health };
...
export function executeInput(state, envelope, sequence, priorReceipts = []) {
  const { action, before, after, mutation } = applyArenaInput(state, envelope);
  ...
  const baseReceipt = { schema: `${SCHEMA}.receipt`, execution_id, sequence, status: 'accepted', generated_at: '1970-01-01T00:00:00.000Z', action_hash, state_before_hash, mutation };
  ...
  const journalBase = { schema: `${SCHEMA}.journal-entry`, execution_id, sequence, canonical_input, canonical_state_before: canonicalize(before), canonical_state_after: canonicalize(after), action, state_before: before, state_after: after, action_hash, receipt_hash: receipt.receipt_hash, mutation, ...commitments };
...
export function replayJournal(journal) {
  ...
  for (const [index, entry] of journal.entries()) {
    const result = executeInput(state, entry.action, index + 1, receipts);
...
export class ArenaHotPocketRuntime {
  ...
  process(input) { const result = executeInput(this.state, input, this.journal.length + 1, this.receipts); this.state = result.state; this.receipts.push(result.receipt); this.journal.push(result.journal); this.persist(); return result; }
