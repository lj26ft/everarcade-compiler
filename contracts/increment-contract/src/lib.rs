...
impl Contract for IncrementContract {
    ...
    fn decode(value: Value) -> Result<Self::Payload, ContractError> {
        serde_json::from_value(value)
            .map_err(|e| ContractError(format!("invalid increment payload: {}", e)))
    }
    ...
    fn execute(
        ...
        let value: i64 = current
            .parse()
            .map_err(|_| ContractError(format!("value is not numeric: {}", current)))?;

        let next = value + 1;

        state.insert(payload.key.clone(), next.to_string());
