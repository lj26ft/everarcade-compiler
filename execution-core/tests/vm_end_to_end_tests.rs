...
fn vm_end_to_end_boundary_flow() {
    ...
    let host_output = HostExecutionOutput { abi_version: CURRENT_ABI_VERSION, vm_output_root: output.vm_receipt_root, encoded_receipt: bincode::serialize(&receipt).unwrap() };
