...
#[test]
fn test_local_tick_equivalence() {
    let a = step_runtime(
        seed_world(),
        vec![RuntimeInput {
            tick: 0,
...
#[test]
fn test_player_movement_replay() {
    let out = step_runtime(
        seed_world(),
        vec![RuntimeInput {
            tick: 0,
            player_id: "player1".into(),
    ...
    assert_eq!(out.world.entities.get(&1).unwrap().y, 1);
}
#[test]
...
fn test_inventory_restore_equivalence() {
    ...
    assert_eq!(a.inventory.ownership.get("player1").unwrap().len(), 1);
}
#[test]
fn test_replay_validation_equivalence() {
    let a = step_runtime(seed_world(), vec![], InventoryState::default());
    assert!(!a.state_root.is_empty());
}
#[test]
fn test_validation_root_progression() {
    let a = step_runtime(seed_world(), vec![], InventoryState::default());
    assert!(!a.validation_root.is_empty());
}
#[test]
fn test_world_persistence_equivalence() {
    let a = step_runtime(seed_world(), vec![], InventoryState::default());
    let ser = serde_json::to_string(&a.world).unwrap();
    let d: WorldState = serde_json::from_str(&ser).unwrap();
    assert_eq!(a.world, d);
}
#[test]
fn test_stdout_event_equivalence() {
    let a = step_runtime(seed_world(), vec![], InventoryState::default());
    let b = step_runtime(seed_world(), vec![], InventoryState::default());
    assert_eq!(a.event_root, b.event_root);
}
#[test]
fn test_large_tick_progression() {
