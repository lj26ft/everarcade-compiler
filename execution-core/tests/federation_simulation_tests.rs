use execution_core::federation::simulation::{
    fixture_catalog, run_fixture, topology_manifest, DivergenceType,
};
...
fn test_multi_node_replay_equivalence() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "5-node federation")
        .unwrap();
...
fn test_archive_sync_equivalence() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "3-node federation")
        .unwrap();
    let report = run_fixture(&fixture, None);
...
fn test_federation_recovery_equivalence() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "restored federation")
        .unwrap();
    let report = run_fixture(&fixture, None);
...
fn test_divergence_detection() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "2-node federation")
        .unwrap();
...
fn test_restoration_replay_equivalence() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "archive recovery federation")
        .unwrap();
    let report = run_fixture(&fixture, None);
    assert!(report
        .validations
        .iter()
        .all(|v| v.replay_root == report.validations[0].replay_root));
}
...
fn test_partition_recovery_equivalence() {
    let fixture = fixture_catalog()
        .into_iter()
        .find(|f| f.federation_id == "partitioned federation")
        .unwrap();
    let report = run_fixture(&fixture, None);
    assert!(report.validations.iter().all(|v| v.equivalent));
}
