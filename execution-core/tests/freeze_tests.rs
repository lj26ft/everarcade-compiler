use execution_core::{
    ...
    hashing, ExecutionPlan, VmInput,
};

#[test]
fn test_abi_frozen() {
    assert_eq!(versions::ABI_VERSION, everarcade_abi::ABI_VERSION);
    assert!(compatibility::is_abi_compatible(
        everarcade_abi::ABI_VERSION
    ));
}
...
fn test_receipt_hash_frozen() {
    ...
    assert_eq!(
        hashing::compute_receipt_hash(&first.receipt),
        hashing::compute_receipt_hash(&second.receipt)
    );
}
