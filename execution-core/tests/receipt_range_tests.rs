use execution_core::{
    merkle::receipt_merkle::receipt_root,
    receipt_runtime::execution_receipt::ExecutionReceipt,
    sync::{validate_receipt_range, ReceiptRange},
};

fn receipt(id: &str, parent: Option<&str>, idx: u64) -> ExecutionReceipt {
    ExecutionReceipt {
        receipt_id: id.into(),
        parent_receipt: parent.map(|s| s.into()),
