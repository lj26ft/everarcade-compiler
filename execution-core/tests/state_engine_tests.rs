...
use execution_core::state_engine::{
    history, merkle, proof, snapshot::StateSnapshot, store::StateStore,
};
...
fn test_state_root_determinism() {
    ...
    assert_eq!(
        merkle::compute_state_root(&entries),
        merkle::compute_state_root(&entries)
    );
}
