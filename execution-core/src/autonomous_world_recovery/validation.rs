...
pub fn validate(state: &AutonomousWorldRecoveryState) -> Result<(), &'static str> {
    let expected = format!(
        "autonomous_world_recovery:{}:continuity:{}:{}:{}",
...
pub fn equivalent(
    a: &AutonomousWorldRecoveryState,
    b: &AutonomousWorldRecoveryState,
) -> Result<(), &'static str> {
    validate(a)?;
    validate(b)?;
    if a == b {
