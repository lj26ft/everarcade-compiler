...
pub fn validate(state: &SocietyRuntimeState) -> Result<(), &'static str> {
    let expected = format!(
        "society_runtime:{}:continuity:{}:{}:{}",
...
pub fn equivalent(a: &SocietyRuntimeState, b: &SocietyRuntimeState) -> Result<(), &'static str> {
    validate(a)?;
    validate(b)?;
    if a == b {
        Ok(())
