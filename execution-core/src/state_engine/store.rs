...
impl StateStore {
    pub fn new(entries: BTreeMap<String, String>) -> Self { Self { entries } }
    pub fn get(&self, key: &str) -> Option<&String> { self.entries.get(key) }
    pub fn set(&mut self, key: String, value: String) { self.entries.insert(key, value); }
    ...
    pub fn root(&self) -> [u8; 32] {
        let ordered: Vec<(String, String)> = self.entries.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        merkle::compute_state_root(&ordered)
    }
    pub fn into_state(self) -> BTreeMap<String, String> { self.entries }
}
