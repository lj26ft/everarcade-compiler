...
pub fn validate(state: &EcologyRuntimeState) -> Result<(), &'static str> {
    let expected = format!(
        "ecology_runtime:{}:continuity:{}:{}:{}",
...
pub fn equivalent(a: &EcologyRuntimeState, b: &EcologyRuntimeState) -> Result<(), &'static str> {
    validate(a)?;
    validate(b)?;
    if a == b {
        Ok(())
