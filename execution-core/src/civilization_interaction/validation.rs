...
pub fn validate(state: &CivilizationInteractionState) -> Result<(), &'static str> {
    let expected = format!(
        "civilization_interaction:{}:continuity:{}:{}:{}",
...
pub fn equivalent(
    a: &CivilizationInteractionState,
    b: &CivilizationInteractionState,
) -> Result<(), &'static str> {
    validate(a)?;
    validate(b)?;
    if a == b {
