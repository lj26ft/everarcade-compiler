use super::{
    fuel::FuelMeter, host_abi::HostFrame, memory_model::MemoryModel, transition::RuntimeTransition,
};
...
pub fn execute_canonical(input: &[u8], initial_fuel: u64) -> Option<RuntimeReceipt> {
    ...
    if !fuel.charge(frame.payload.len() as u64) {
        return None;
    }
    let transition = RuntimeTransition::apply(
        0,
    ...
    Some(RuntimeReceipt {
        trace_hash: transition.state_after,
        final_memory_hash: memory.digest(),
