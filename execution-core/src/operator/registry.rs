...
pub fn save_recovery_descriptor(path: &Path, descriptor: &WorldRecoveryDescriptor) -> Result<(), OperatorRecoveryError> {
    ...
    let bytes = canonical_encode(&stored).map_err(|e| OperatorRecoveryError::Storage(e.to_string()))?;
...
pub fn load_recovery_descriptor(path: &Path) -> Result<WorldRecoveryDescriptor, OperatorRecoveryError> {
    ...
    if computed != stored.descriptor_hash {
        return Err(OperatorRecoveryError::Validation(super::continuity::OperatorRecoveryMismatch {
